...
function characterclick3();
	if(@task1010==1);  
		...
		RPGCharacter.move(1,4,0);
		RPGCharacter.move(2,4,0);
		RPGTalk.set(1,0,好，移动测试结束，下面测试动作变换。首先关羽，向左转，原地踏步！);
		RPGCharacter.changeAction(2,move,left);
		RPGTalk.set(1,0,立定！);
		RPGCharacter.changeAction(2,stand,left);
		RPGTalk.set(1,0,接下来，跟我互相对视！);
		RPGCharacter.changeAction(1,stand,2);
		RPGCharacter.changeAction(2,stand,1);
		...
		RPGCharacter.changeAction(1,stand,3);
		RPGCharacter.changeAction(3,stand,1);
